...
#    RSYNC_PASSWORD="shira@mars.dbcls.jp" rsync -avz --delete --include "*/" --include "*.xml" --exclude "*" anonymous@ftp.ncbi.nlm.nih.gov::sra/Submissions/* $DIR
    ...
    lftp -e 'mirror --only-newer --verbose -i .tab /sra/reports/Metadata/ /share/data/sra_meta/; quit'  ftp://ftp-trace.ncbi.nlm.nih.gov/ 1> "./log/meta_mirror.$DATE.log" 2> "./log/meta_error.$DATE.log"
...
#    lftp -f ./geomirror.lftp 1> "./log/geo-mirror.$DATE.log" 2> "./log/geo-error.$DATE.log"
    ...
    lftp -e 'mirror --delete --only-newer --verbose /ddbj_database/biosample/ /share/data/biosample/ddbj/; quit' ftp://ftp.ddbj.nig.ac.jp/ 1> "./log/biosm-ddbj-mirror.$DATE.log" 2> "./log/biosm-ddbj-error.$DATE.log"

    lftp -e 'mirror --delete --only-newer --verbose /biosample/ /share/data/biosample/ncbi/; quit' ftp://ftp.ncbi.nlm.nih.gov/ 1> "./log/biosm-ncbi-mirror.$DATE.log" 2> "./log/biosm-ncbi-error.$DATE.log"


    lftp -e 'mirror --only-newer --verbose -i .tar.gz -x .md5 /pub/taxonomy/ /share/data/taxonomy/; quit' ftp://ftp.ncbi.nlm.nih.gov/
